...
public class Documentor {
	...
	private void generateDoc(OxFile oxFile, String fileName) throws Exception {
		...
		try {
			...
			generateClassDetailDocs(output, null,
					oxFile.functionsAndVariables(), oxFile.enums());
	...
	private void generateGlobalHeaderDocs(OutputFile output,
		...
		ArrayList[] visMembers = { oxfile.variables(), oxfile.functions(),
				oxfile.enums() };
		...
		if (table.getRowCount() > 0) {
			Header header = new Header(oxdoc, 2, iconType, "Global "
					+ sectionName);
	...
	private void generateEnumDocs(OutputFile output, OxClass oxclass,
		...
		for (int i = 0; i < memberList.size(); i++) {
			...
			String[] row = {
					new Anchor(oxdoc, anchorName).toString()
							+ entity.displayName(),
